...
Although the coefficients for BCPI components are relatively low, this could be attributed to the large range and average values of these indices as shown in @fig-data-sum. The **weekly energy BCPI** has a broad span, with values ranging from 874.12 to over 2282.35. Thus, these results still indicate that commodity prices have a noticeable influence on the exchange rate.
...
#| warning: false
#| label: fig-vif1
#| fig-cap: "Variance Inflation Factor (VIF) values for all predictors in the first model are below 5, indicating that multicollinearity is not a concern in these models."
#| layout-ncol: 1
...
#| warning: false
#| label: fig-vif2
#| fig-cap: "Variance Inflation Factor (VIF) values for all predictors in the second model are below 5, indicating that multicollinearity is not a concern in these models."
#| layout-ncol: 1
...
2. **Expanded Commodity Price Data**:  
   ...
   **Variables Collected**:  
   ...
   - **Commodity index values**: GCPI and sector-specific price indices for better market coverage.
